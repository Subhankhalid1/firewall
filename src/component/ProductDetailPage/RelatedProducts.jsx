...
export default function App({ productCategory }) {
    ...
    const { feature, slug } = queryString.parse(window.location.search);
    const breakPoints = [
    ...
    return (
        <div className="container mb-5">
            <section className="text-center bygga-parent equip-parent pt-5 pb-4">
                <span className="bygga-text-heading fs-4 fw-bold text-muted text-capitalize">
                    related products
                    </span>
            </section>
                ...
                <Carousel breakPoints={breakPoints} pagination={false}>
                    {
                        productCategory?.map((item, index) => {
                            return <section key={index} className="bg-white mb-3 mx-2 shadow px-2 py-4 " style={{ borderRadius: "15px" }}>
                                ...
                                <div className="pb-3">
                                    ...
                                    <button onClick={() => addToCart(item)} className="btn btn-primary mt-3 btn-size">
